  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      if (productExit) {
       ...
       item.id === action.payload.product.id  ? { ...productExit, qty: productExit.qty+ action.payload.num }: item
        );
      } else {
    ...
    emptyCart: (state) => {
      // This reducer will reset cartList to an empty array
      state.cartList = [];
    }
  },
});
...
export const { addToCart, decreaseQty, deleteProduct ,emptyCart} = cartSlice.actions;
